...
WINDOWS-FIXED VERSION - Properly handles UTF-8 encoding
...
class FlakeMetricsCollector:
    ...
    def count_lines_of_code(self, filepath):
        ...
        try:
            # FIXED: Use UTF-8 encoding explicitly with error handling
    ...
    def run_flake8_basic(self):
        """Run basic Flake8 analysis - WINDOWS-FIXED VERSION"""
        print(f"\n{'='*60}")
        ...
        for filepath in PYTHON_FILES:
            ...
            # Run Flake8 - FIXED: Proper encoding handling for Windows
            try:
                # CRITICAL FIX: Set encoding and use text=True with explicit UTF-8
                result = subprocess.run(
                    ['flake8', filepath, '--format=%(path)s:%(row)d:%(col)d: %(code)s %(text)s'],
                    capture_output=True,
                    text=True,
                    encoding='utf-8',  # ADDED: Force UTF-8
                    errors='replace'   # ADDED: Replace problematic characters
                )
                ...
                if result.stderr:
                    print(f"   ⚠️  Flake8 warnings: {result.stderr[:100]}")
                ...
                print(f"   → File contains non-ASCII characters that couldn't be decoded")
                print(f"   → Attempting with error handling...")
                
                # Retry with more aggressive error handling
                try:
                    result = subprocess.run(
                        ['flake8', filepath],
                        capture_output=True,
                    ...
                    if result.stdout:
                        violations = result.stdout.strip().split('\n')
                        self.metrics['violations_by_file'][filepath] = len([v for v in violations if v.strip()])
                        print(f"   ⚠️  Partial analysis: {len(violations)} violations (with encoding workaround)")
                    else:
                        self.metrics['violations_by_file'][filepath] = 0
                except Exception as e2:
                    print(f"   ❌ Even with workaround, analysis failed: {e2}")
                    self.metrics['violations_by_file'][filepath] = -1  # Mark as failed
                    continue
    ...
    def save_daily_summary(self):
        ...
        # FIXED: Use UTF-8 encoding for CSV
    ...
    def save_detailed_violations(self):
        ...
        with open(violations_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
    ...
    def generate_report(self):
        ...
        for filepath, count in sorted(
            ...
            key=lambda x: x[1] if x[1] >= 0 else 0,  # Handle -1 (failed files)
            reverse=True
        )[:5]:
            if count >= 0:
                print(f"  {filepath}: {count}")
            else:
                print(f"  {filepath}: ANALYSIS FAILED")
...
def main():
    ...
    # Set UTF-8 mode for Windows console output
